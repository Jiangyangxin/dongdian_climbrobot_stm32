    ...
    void fanTask(void *pvParameters)
    {
        ...
        while (true)
        {
            ...
            if (fanCnt % fanTick == 0)
            {
                ...
                if (pdPASS == xTaskNotifyWait(0x00000000, 0x00000000, &notification_value, 0)) //四个参数分别代表： 任务通知初始值为0，接受通知后通知清零，不阻塞
                {
                    fanVal->fan_speed = 1000000.f / (float)notification_value; //采样频率是 1000000hz ，假如采集到的计数值为 100，说明 每100个tick转了一圈，因此角速度为 1000000/100 （圈/s）
                }
            }
