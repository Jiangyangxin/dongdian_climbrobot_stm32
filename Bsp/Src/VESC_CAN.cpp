...
void comm_can_transmit_eid(uint32_t id,  uint8_t *data, uint8_t len) 
{
	...
	{
		TransmitMailbox = HAL_CAN_AddTxMessage(&hcan1,&TxMessage,data,&Tx_MailBox);
	}
}
   ...
   void VESC_Task(void *pvParameters)
	{
		...
		while (true)
		{
			if(io_state_cmd==1)
			{
			comm_can_set_rpm(VESC_SANDING_ID,3000);
			}
			...
			comm_can_set_rpm(VESC_SANDING_ID,5000);
						...
						else if(io_state_cmd==4)
			{
			comm_can_set_rpm(VESC_SANDING_ID,6000);
			}
						else if(io_state_cmd==5)
			{
			comm_can_set_rpm(VESC_SANDING_ID,7000);
			}
						else if(io_state_cmd==6)
			{
			comm_can_set_rpm(VESC_SANDING_ID,8000);
			}
						else if(io_state_cmd==7)
			{
			comm_can_set_rpm(VESC_SANDING_ID,9000);
			}
						else if(io_state_cmd==8)
			{
			comm_can_set_rpm(VESC_SANDING_ID,10000);
			}
						else if(io_state_cmd==9)
			{
			comm_can_set_rpm(VESC_SANDING_ID,12000);
			}
						else if(io_state_cmd==10)
			{
			comm_can_set_rpm(VESC_SANDING_ID,14000);
			}
						else if(io_state_cmd==11)
			{
			comm_can_set_rpm(VESC_SANDING_ID,16000);
			}
						else if(io_state_cmd==12)
			{
			comm_can_set_rpm(VESC_SANDING_ID,18000);				
			}			
			else if(io_state_cmd>12)
			{
			comm_can_set_rpm(VESC_SANDING_ID,10000);
